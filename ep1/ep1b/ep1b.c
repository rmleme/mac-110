 ...
 * Descricao: este programa simula o conhecido jogo NIM. Nesta versao, um   *
 *            ser humano joga contra o computador (representado pelo Prof.  *
 *            Teoria), comecando de uma configuracao inicial (definida pelo *
 *            usuario) ate o momento em que nao reste nenhum palito em jogo.*
 ***************************************************************************/
...
char escolhe_pri_jogador(void)
{
  ...
  {
    escrevexy(1,4,"Qual jogador sera o primeiro, 'L' (aluno) ou 'R' (Prof. Teoria)? ");
	 scanf("%c",&pri_jogador);
	 gotoxy(43,4);
	 clreol();
  }
...
void main(void)
{
  ...
  {
    if (jogador_atual == 'L')      /* E o turno do aluno */
    {
	   rodape(jogador_atual,", faca sua jogada.");
		le_lance(&qual_monte,&palitos_a_deixar);
    }
    else                           /* E o turno do Prof. Teoria */
    {
      qual_monte       = lance_prof_Teoria(monte[0],monte[1],monte[2],monte[3],monte[4],0);
      palitos_a_deixar = lance_prof_Teoria(monte[0],monte[1],monte[2],monte[3],monte[4],1);
      mensagem("Lance do Prof. Teoria: pressione algo para continuar");
    }
  ...
  if (vencedor == 'L')             /* O aluno venceu */
    mensagem("Parabens aluno, voce derrotou o Prof. Teoria!");
  else                             /* O Prof. Teoria venceu */
    mensagem("Mais uma vez o Prof. Teoria venceu!");
  textcolor(LIGHTGRAY);
